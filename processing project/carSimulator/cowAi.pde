void cowAI(car t){
  ...
  if(d.len>200){
    if(d.projection(cclomen.cowAngle).len>=-500) cclomen.speed+=0.01;
    else cclomen.speed-=0.01;
...
void carAI(cow t){
  ...
  if(d.len>200){
    ...
    if(d.projection(c.carAngle+c.wheelAngle*c.tk+HALF_PI).len>0) c.wheelAngle+=0.006;

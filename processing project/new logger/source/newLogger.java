...
public class newLogger extends PApplet {
...
public void draw(){
  t++;
  background(255);
  loggers[0].add(sin(t*0.01f)*1000);
  loggers[1].add(cos(t*0.01f)*1000);
  loggers[2].add(mouseX-width*0.5f);
  loggers[3].add(mouseY-height*0.5f);
...
class logger {
  ...
  public void draw() {
    ...
    if (mousePressed) {
      ...
      if (h>(int)(maxdata*0.618f)) {
        if(mdy!=-1)y+=h-(int)(maxdata*0.618f);
        h=(int)(maxdata*0.618f);
      }
      if (w<50) {
        if(mdx!=-1)x-=50-w;
        w=50;
    ...
    if (rew&&mdx==-1) rect(x+w-2.5f, y, 5, h);
    if (rew&&mdx!=-1) rect(x-2.5f, y, 5, h);
    if (reh&&mdy!=-1) rect(x, y-2.5f, w, 5);
    if (reh&&mdy==-1) rect(x, y+h-2.5f, w, 5);
  ...
  public void add(float f) {
    ...
    if (vmax*scale>h&&scale*0.5f>=0.01f)scale*=0.5f;
  }
  public void savedata() {
    savedata(start, len);
  }
  public void savedata(int s, int l) {
  ...
  public void snap(logger[] loggers){
...
public float range(float min,float max,float f){
  return max(min,min(max,f));
}
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "newLogger" };
    if (passedArgs != null) {
